...
# TODO: code to store sim data using a sim_data class
    ...
    def gain_term(self, plan_exp, params):
        ...
        for i, exps  in enumerate(plan_exp):
            ...
            for j, e in enumerate(exps): # remember a single experience e is (s, a , r , s_next)
                ...
                Q_pre = Q_mean.copy()
                # Policy before
                prob_a_pre = np.exp(Q_mean / params.tau) / np.sum(np.exp(Q_mean))
                ...
                steps_to_end = len(exps) - j # remaining steps to end of trajectory
                rew_to_end = np.sum(params.gamma ** np.arange(steps_to_end) * exps[j: , 2])
                Q_target = rew_to_end + params.gamma ** (steps_to_end + 1) * s_next_val
                Q_mean[a_e] = params.alpha * (Q_target - Q_mean[a_e])

                # Policy after backup
                prob_a_post = np.exp(Q_mean / params.tau) / np.sum(np.exp(Q_mean))
        ...
        for plan in range(params.planning_steps):
            ...
            if params.expand_further and len(planning_backups) > 0:
                ...
                if not s_n >= self.mdp.nb_states:
                    ...
                    s_n_p1 = self.exp_last_s_next[s_n, a_n] # Resulting state from taking action an in state sn
                    ...
                    next_step_is_repeated = s_n_p1 in seq_so_far[:, 0] or s_n_p1 in seq_so_far[:, 3] # Check whether a loop is formed. Bool as well
                    ...
                    if not next_step_is_nan and (params.allow_loops or not next_step_is_repeated): # If loops are not allowed and next state is repeated, don't expand this backup
                        seq_updated = np.concatenate((seq_so_far, np.array([s_n, a_n, r_n, s_n_p1]).reshape(1, 4)), axis = 0)
                        plan_exp.append(seq_updated)
                        # plan_exp.append(seq_updated)
                        # plan_exp = np.array(plan_exp)
                        # print(seq_updated)
            ...
            if np.max(EVM) > EVM_thresh:
                ...
                max_EVM_idx = np.where(EVM == max(EVM))[0]
                ...
                max_EVM_idx = int(max_EVM_idx)
                for n, exp in enumerate(plan_exp[max_EVM_idx]):
                    s_plan = int(exp[0])
                    a_plan =int(exp[1])
                    s_next_plan = int(plan_exp[max_EVM_idx][-1, -1])
                    rew_to_end = plan_exp[max_EVM_idx][n:, 2]
    ...
    def update_transi(self, s, s_next, params):
        if s not in self.mdp.terminal_states:
            ...
            self.T[s, : ] += params.T_learning_rate  * (target_vector - self.T[s, :])
