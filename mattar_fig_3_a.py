...
for k, data in enumerate(list_data):
    data.replay.state = np.array(data.replay.state, dtype = int).T
    data.replay.action = np.array(data.replay.action, dtype = int).T
    candidate_events = get_candidate_events(data, mdp, min_frac_cells, min_num_cells)
    lap_num = np.array(data.num_episodes) # episode number for each time point
    lap_num_events = lap_num[candidate_events] # episode number for each candidate event
    ...
    for e, evt in enumerate(candidate_events):
        event_state = np.array(data.replay.state[evt], dtype = int) # In a multi-step sequence, simData.replay.state has 1->2 in one row, 2->3 in another row, etc
        event_action = np.array(data.replay.action[evt], dtype = int) # In a multi-step sequence, simData.replay.action has the action taken at each step of the trajectory
        ...
        for i, evt_state in enumerate(event_state[:-1]):
            ...
            if next_state[event_state[i], event_action[i]] == event_state[i +1]:
                event_dir[i] = "F"

            # If state(i+1) and action(i+1) leads to state(i): REVERSE
            if next_state[event_state[i + 1], event_action[i + 1]] == event_state[i]:
                event_dir[i] = "R"

            if event_dir[i] == "": # If this transition was neither forward nor backward
                break_points.append(i) # Then, call this a breakpoint

            elif i > 0:
                if event_dir[i] != event_dir[i - 1]:
                    break_points.append(i)

            if (i + 1) == (len(event_state) - 1):
                break_points.append(i + 1) # Add a breakpoint after the last transition

        # Break this event into segments of sequential activity
        for j, b_pt in enumerate(break_points[:-1]):
            this_chunk = list(range(break_points[j] + 1, break_points[j + 1]))
...
ax.bar(lab,to_plot)
plt.show()
plt.savefig("results/Mattar/mattar_fig_3_a_default.png")
